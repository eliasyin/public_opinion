...
class SelfAttention(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # 　attention_mask : zero_tensor, 这里送去了一个全0tensor, 这个参数并没有起到任何作用
        SA_out = self.SA(inputs, zero_tensor)
        return self.tanh(SA_out[0])  # (batch, max_seq_len, embed_dim)


class LCF_APC(BertForTokenClassification):

    def __init__(self, bert_base_model, args):
        super(LCF_APC, self).__init__(config=bert_base_model.config)
