...
import copy
import numpy as np
from utils.train_utils import mySequential
...
class SelfAttention(nn.Module):
    ...
    def forward(self, inputs):
        # zero_vec shape : (batch_size, 1, 1, max_seq_len)
        zero_vec = np.zeros((inputs.size(0), 1, 1, self.opt.max_seq_length))
        zero_tensor = torch.tensor(zero_vec).float().to(
            self.opt.device)  # zero_tensor?
        # 　attention_mask : zero_tensor, 这里送去了一个全 0 tensor, 这个参数并没有起到任何作用
        SA_out = self.SA(inputs, zero_tensor)
        return self.tanh(SA_out[0])  # (batch, max_seq_len, embed_dim)


class LCF_APC_ADJ(BertForTokenClassification):

    def __init__(self, bert_base_model, args):
        super(LCF_APC_ADJ, self).__init__(config=bert_base_model.config)
    ...
    def cal_feature(self, dep_adjs, rel_adjs, dist_to_aspect):
        
        pass

    def forward(self, input_ids_spc, token_type_ids=None, attention_mask=None, labels=None, polarities=None, valid_ids=None,
                attention_mask_label=None, dep_adjs=None, rel_adjs=None, adj_masks=None, pos_tag_idx=None, dist_to_aspect=None):
        ...
        # 提取出aspect的属性标签，注意数据已经处理过，所以每次一个样本只有一个属性
